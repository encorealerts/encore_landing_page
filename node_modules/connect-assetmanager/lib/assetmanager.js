				...
				Step(function () {
					...
					if (!group.debug) {
						if (dataTypeLowerCase === 'javascript' || dataTypeLowerCase === 'js') {
							...
							var captures = content.match(/url\([^(]*\)/ig), files;
							if (captures) {
							  files = captures.map(function (cap) {
							    return {
							      cdn: cap.replace(/url\('/,'url(\'https://s3.amazonaws.com/encore-landing-page').replace(/url\([^']+/,'url(https://s3.amazonaws.com/encore-landing-page'),
							      original: cap
							    }
							  });
							}
							files.forEach(function (cap) {
								content = content.replace(cap.original, cap.cdn);
							});
